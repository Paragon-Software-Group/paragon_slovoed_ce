#ifndef __MORPHO_DATA_INCLUDE_V2__
#define __MORPHO_DATA_INCLUDE_V2__
...
/// Савельев: используем 4 байта
typedef UInt32 STAT_VECTOR_SIZE_V2;
...
#define M_ASSERT(x)					
	...
	///
	//////////////////////////////////////////////////////////////////////
	template <typename _DATA> class stat_vector_v2
	{
	private:

		/// private data (const only)

		const STAT_VECTOR_SIZE_V2	bsize;		/** размер массива в байтах */
...
//			sldMemZero(array, 0, sizeof(array));
		...
		inline STAT_VECTOR_SIZE_V2 size(void) const { return (MORPHO_SWAP_32(bsize)/sizeof(_DATA)); };
		...
		inline const_iterator end(void)   const { return (const_iterator((_DATA*)((char*)array + MORPHO_SWAP_32(bsize)))); };

		/// Возвращает элемент массива по инексу. @attention Не проверяет на предмет выхода за границы.
		inline const _DATA & operator[](STAT_VECTOR_SIZE_V2 index) const { return array[index]; };

		/// additional function

		/// Возвращает размер массива в байтах.
		inline STAT_VECTOR_SIZE_V2 size_in_bytes(void) const { return (MORPHO_SWAP_32(bsize) + sizeof(STAT_VECTOR_SIZE_V2)); };

	};

	///	Наш внутренний базовый внутренний тип - линейный контейнер элементов
	#define MVECTOR_V2 stat_vector_v2
#endif
...
public:
	...
	inline bool operator<(const MCHARREF_V2& other) const { return (ptr < other.ptr) ? true : false ; };
#else
	///////////////////////////////////////////////////////
	//
	/// core version
...
private: 
	...
	/// Четырехбайтная ссылка внутри файла
	const STAT_VECTOR_SIZE_V2 shift;
...
public:
	/// default constuctor
	inline OBJREF_V2() { ptr = NULL; };

	/// other constuctor
	inline OBJREF_V2(const _TYPE* strPtr) { ptr = strPtr; };
	...
	/// на базовый тип
	inline bool operator==(const OBJREF_V2& other) const { return (ptr == other.ptr) ? true : false ; };
	inline bool operator!=(const OBJREF_V2& other) const { return (ptr != other.ptr) ? true : false ; };
...
public:
	...
	inline bool operator==(OBJREF_V2& other) { return (shift == other.shift) ? true : false ; };
	/// Сравнение другой ссылкой этого же типа без преобразования к указателю на базовый тип
	inline bool operator!=(OBJREF_V2& other) { return (shift != other.shift) ? true : false ; };
...
struct MorphoInflectionRule_v2;
struct MorphoInflectionRulesSet_v2;
struct MorphoStateDescription_v2;
struct MorphoStateDescriptionTable_v2;
class  RulesByLastChar_v2;

#include "WordSet_v2.h"
...
struct MorphoStateDescription_v2
{
	...
	MCHARREF_V2 description;

	/// Additional info for this rule (set only if table=\b NULL)
	MCHARREF_V2 templ;

	/// Additional info for this rule (set only if table=\b NULL)
	MCHARREF_V2 question;

	/// Link to other table
	OBJREF_V2 <const MorphoStateDescriptionTable_v2> table;
...
struct MorphoStateDescriptionTable_v2
{
	...
	typedef MVECTOR_V2 <MorphoStateDescription_v2> StatesList;
	StatesList states;
	...
	inline const MorphoStateDescription_v2& StateByIndex (Int32 index) const { return states[index]; };
...
struct MorphoInflectionRule_v2
{
	...
	/// строки - по нечетным.
	//
	/// Является ли это правило ссылкой на поднибор правил?
	inline bool IsLinkToRuleset(void) const { return (MORPHO_SWAP_32(shift)&1) ? false: true; };
#endif
	...
	/// при хранении
	bool operator < (const struct MorphoInflectionRule_v2 &other) const
	{
...
public:
	/// Конструктор.
	MorphoInflectionRule_v2::MorphoInflectionRule_v2()
	{
		MorphoInflectionRule_v2::shift = 0;
	}
*/
};
...
struct MorphoInflectionRulesSet_v2
{
	...
	MCHARREF_V2	precondition;

	/// Набор правил
	typedef MVECTOR_V2 <MorphoInflectionRule_v2> RulesList;
	RulesList rules;
	...
	//////////////////////////////////////////
		...
		const MorphoStateDescriptionTable_v2* table;

		/// точно такой же рулесет, но, находящийся в set (для того, чтобы убедиться в уникальности)
		MorphoInflectionRulesSet_v2 *shadow;

		/// оператор сравнения для того, чтобы отсекать повторяющиеся элементы
		/// при хранении
		bool operator < (const struct MorphoInflectionRulesSet_v2 &other) const
		{
...
struct MorphoClass_v2
{
	...
	typedef MVECTOR_V2 < const OBJREF_V2<MorphoInflectionRulesSet_v2> > InflTablesList;
...
public:
	...
	struct OneCallParameters
	{
		...
		const MorphoInflectionRulesSet_v2* ruleset; /// ссылка на сам набор правил, откуда это все взято
	};
#else
	struct OneCallParameters
	{
		MCHARREF_V2	precondition;				/// ссылка на предусловие
		MCHARREF_V2	rule;						/// ссылка на правило
		...
		OBJREF_V2<MorphoInflectionRulesSet_v2>	ruleset; /// ссылка на сам набор правил, откуда это все взято
	};
#endif

	/// Конструктор
	RulesByLastChar_v2(void)
	{
		sldMemZero(&lastCharMap, sizeof(lastCharMap));
		sldMemZero(&callParams, sizeof(callParams));
	};

	//////////////////////////////////////////////////////////////////////
	///
	/// Итератор для удобного хранения промежуточных состояний при 
	/// поиске подходящего правила.
	///
	//////////////////////////////////////////////////////////////////////
...
private:
	...
	/// add single sule
	void AddSingleRule(const MCHAR* precondition, const MCHAR* rule, const MorphoInflectionRulesSet_v2 *ruleset);
...
public:
	...
	void AddRuleset(const MorphoData_v2& morpho, const MorphoInflectionRulesSet_v2 *ruleset, const MorphoInflectionRulesSet_v2 *base_ruleset = 0);
	...
	void AddBlock(OneCallParameters *block, UInt32 count)
	{
	...
	const OneCallParameters *GetParamByIndex(UInt32 index) const
	{
		UInt32 resIdx;
		UInt32 localIdx;
...
///
///////////////////////////////////////////////////////////////////////
struct MorphoData_v2 : public IMorphoData
{
#ifndef _ML_PARSER_

	// хендлеры загруженных ресусров
	sld2::DynArray<UInt8>	m_resText;
	sld2::DynArray<UInt8>	m_resClass;
	CSDCReadMy::Resource	m_resArchHeader;
	sld2::DynArray<UInt8>	m_resRules2;
	sld2::DynArray<UInt8>	m_resRulesSet;
	sld2::DynArray<UInt8>	m_resQAIndex;
	CSDCReadMy::Resource	m_resTree;
	sld2::DynArray<UInt8>	m_resTable;
	sld2::DynArray<UInt8>	m_resLastChar;
...
*/
	...
	inline const MorphoStateDescriptionTable_v2* ObjRef2Ptr(OBJREF_V2<const MorphoStateDescriptionTable_v2> ref) const { return (const MorphoStateDescriptionTable_v2*)( (char*)m_resTable.data() + MORPHO_SWAP_32(ref.shift) ); };

	/// Преробразование ссылки на набор правил в указатель
	inline const MorphoInflectionRulesSet_v2* ObjRef2Ptr(OBJREF_V2<const MorphoInflectionRulesSet_v2> ref) const { return (const MorphoInflectionRulesSet_v2*)( (char*)m_resRulesSet.data() + MORPHO_SWAP_32(ref.shift) ); };
	inline const MorphoInflectionRulesSet_v2* ObjRef2Ptr(OBJREF_V2<MorphoInflectionRulesSet_v2> ref) const { return (const MorphoInflectionRulesSet_v2*)( (char*)m_resRulesSet.data() + MORPHO_SWAP_32(ref.shift) ); };

	/// Преробразование ссылки на строку в указатель
	inline const MCHAR* McharRef2Ptr(MCHARREF_V2 ref) const { return (const MCHAR*)( (char*)m_resText.data() + MORPHO_SWAP_32(ref.shift) ); };

	/// Преобразование элемента таблицы правил к указателю на текстовое правило или \b NULL
	inline const MCHAR* Rule2MCharPtr(struct MorphoInflectionRule_v2 ruleref) const 
	{ 
		M_ASSERT( ! ruleref.IsLinkToRuleset() );
		return (const MCHAR*)( (char*)m_resText.data() + MORPHO_SWAP_32(ruleref.shift) );
	};

	/// Преробразование элемента таблицы правил к указателю на поднабор правил или \b NULL
	inline const MorphoInflectionRulesSet_v2* Rule2RuleSetPtr(struct MorphoInflectionRule_v2 ruleref) const
	{
		M_ASSERT( ruleref.IsLinkToRuleset() );
		return (const MorphoInflectionRulesSet_v2*)( (char*)m_resRulesSet.data() + MORPHO_SWAP_32(ruleref.shift) );
	};
	...
	/// Хранилище классов
	MVECTOR_V2<MorphoClass_v2> classes;
	...
	inline const MCHAR* McharRef2Ptr(MCHARREF_V2 ref) const { return ref.ptr; };

	/// преробразование элемента таблицы правил к указателю на текстовое правило или \b NULL
	inline const MCHAR* Rule2MCharPtr(struct MorphoInflectionRule_v2 ruleref) const
	{ 
		M_ASSERT( ! ruleref.IsLinkToRuleset() );
	...
	{
		M_ASSERT( ruleref.IsLinkToRuleset() );
		return (const MorphoInflectionRulesSet_v2*)ruleref.ptr;
	};
	...
	/// Класс, овечающий за ускоренное востановление
	/// базовой формы слова из произвольной.
	/// @see RulesByLastChar
	RulesByLastChar_v2 lastCharMap;
...
public:
	...
	LanguageSpecificData_v2 langSpecData;
	...
	#ifdef _ML_PARSER_	
		typedef MVECTOR_V2<MorphoClass_v2>::const_iterator ClassesIterator;
	#else
		typedef STAT_VECTOR_SIZE_V2 ClassesIterator;
	#endif
	...
	const MorphoClass_v2* GetNextClass (ClassesIterator &iter) const
	#ifdef _ML_PARSER_	
		{
	...
	#else
		{
		const MorphoClass_v2* classPtr;

			// достигли конца?
			if ( iter >= classes_end )
				return (MorphoClass_v2*)0;

			classPtr = (const MorphoClass_v2*)( (char*)m_resClass.data() + iter );

			// следующий класс в списке
			iter += ((const MorphoClass_v2*)( (char*)m_resClass.data() + iter ))->iflectionTables.size_in_bytes() // размер вектора
					+ (STAT_VECTOR_SIZE_V2)(UInt4Ptr)(&((MorphoClass_v2*)0)->iflectionTables);  // смещение вектора от начала структуры
	...
	private:
		...
		// проверок
		const MorphoInflectionRulesSet_v2* simultaneousFormsRulesets[MAX_SAVED_FORMS];
		...
		const MorphoInflectionRulesSet_v2 * foundFormsRulesets[MAX_SAVED_FORMS];
		...
		Int32 saveFoundForm(const MCHAR * form, const MorphoInflectionRulesSet_v2 * rs)
		{
			if (foundFormsNumber >= MAX_SAVED_FORMS) return -1;
			StrNCopy((Int8*)foundForms[foundFormsNumber], (Int8*)form, MAX_WORD_LEN);
...
private:
	...
	enum
	{
		M_DATABASE_FORMAT_MIN_VERSION = 106,
		M_DATABASE_FORMAT_MAX_VERSION = 107
	};
