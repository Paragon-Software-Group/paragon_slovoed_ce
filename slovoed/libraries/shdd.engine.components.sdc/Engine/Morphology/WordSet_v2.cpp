...
void WordSet_v2::SortList(s_word *WordList, Int32 index)
{
	s_word tempdata;
...
void WordSet_v2::CreatePrefixList(s_word *WordList, Int32 index, Int32 maxrulenum)
{
	s_str pattern;
	s_str temp;
...
void WordSet_v2::Init(const void *ArchHeader,
				   const CSDCReadMy::Resource *ArchivePtrArray,
				   const void	*IndexArrayPtr,
				   const STAT_VECTOR_SIZE_V2 *RulesSetPtr,
				   const UInt8	*HuffmanTreePtr,
				   const MorphoData_v2 *morphoPtr)
{
...
bool WordSet_v2::SearchWord(const MCHAR *word, WordRuleSetsIterator &iter, 
						 MCHAR * partialFormBuffer, Int32 bufSize, 
						 const MorphoInflectionRulesSet_v2 * rsArray, Int32 rsArrSize) const
{
#ifdef _ML_PARSER_
	std::string s(word);
...
//	Int32		prev_word_bit = 0;
	...
	for(Int32 mult =  RuleSetArraySize/sizeof(STAT_VECTOR_SIZE_V2) ; mult; digits_for_rule++, mult /= 10);
	...
	while(1)
	{
		...
		for (i = 0; i < 4; i++)
			t[i] = IndexArray[med].w[i];
		t[4] = 0xFF;
		t[5] = 0;
		if(CSldCompare::StrCmpA((const UInt8*)t, (const UInt8*)word) > 0)
			rightborder = med;
		else
			leftborder = med;
	}
	...
	// Савельев: начиная с версии 107 в старшем байте количество слов не записывается,
	// поскольку в ядре при декодировании это значение никак не используется. Старший байт также используется
	// для хранения номера бита (снимается ограничение на размер архива в 0x00FFFFFF бит - 2 Mb)
	...
	while(1)// 
	{
				...
				// с которого начинаются правила для данного слова. 
			{
				if(digitcount == 0)// цифра первая, значит длина префикса
				{
					...
					len = prev_word_len - letterindex;
					if (len > 0)
						sldMemZero(pattern+letterindex, len);
					
					if (!partialFormBuffer || !rsArray)
					{
						cmp = CSldCompare::StrCmpA((UInt8 *)word, (UInt8 *)pattern);
					...
					{ // Если нас интересует частично совпадающее слово
						len = CSldCompare::StrLenA(pattern);
						cmp = sld2::StrNCmp((UInt8*)word, pattern, len);
						...
						if (cmp == 0)
						{
							...
							if (success)
							{
								CSldCompare::StrNCopyA((UInt8*)partialFormBuffer, pattern, bufSize); 
								break;
							}
						}
					}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
bool WordSet_v2::EnumWords(EnumWordsCallBackT* callback, void* param) const
{
#ifdef _ML_PARSER_
	return true;
/*	std::string s(word);
...
#ifdef _ML_PARSER_
bool WordSet_v2::Insert(const MCHAR *word,  MorphoInflectionRulesSet_v2* Mo)
{
	std::string s(word);

	std::map<std::string,std::vector <MorphoInflectionRulesSet_v2 *> > ::iterator itr;
	...
	{
		std::vector <MorphoInflectionRulesSet_v2 *> a;
		a.push_back(Mo);
...
///////////////////////////////////////////////////////////////////////////////////
#ifdef _ML_PARSER_
	const MorphoInflectionRulesSet_v2* WordSet_v2::GetNextRuleSet(WordRuleSetsIterator &iter)const
	{
...
const MorphoInflectionRulesSet_v2* WordSet_v2::GetNextRuleSet(WordRuleSetsIterator &iter)const
	{
		...
		const STAT_VECTOR_SIZE_V2 *temp = iter.ptr;
		...
		else 
		{
			...
			if( (*temp) & MORPHO_SWAP_32(0x00000001))
			{
				iter.ptr = NULL;
				return 0;
			}
		...
		STAT_VECTOR_SIZE_V2 offset = ( (*temp) & MORPHO_SWAP_32(0xFFFFFFFE) );

		return MorphoPtr->ObjRef2Ptr( *(OBJREF_V2<const MorphoInflectionRulesSet_v2> *)&offset );
