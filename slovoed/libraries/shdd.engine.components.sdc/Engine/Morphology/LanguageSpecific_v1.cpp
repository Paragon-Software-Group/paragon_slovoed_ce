...
LanguageSpecificData_v1::LanguageSpecificData_v1()
{
...
UInt16 LanguageSpecificData_v1::FillPronounArrays()
{
...
Int32 LanguageSpecificData_v1::GetPronounsVectorSize(bool numb)const{
...
const MCHAR * LanguageSpecificData_v1::GetRevesedPronoun(bool vectNumb, Int32 pronNumb)const{
	return GetPronounForm(vectNumb, pronNumb);
}

// Выдаем базовую форму местоимения (из одного из массивов)
const MCHAR * LanguageSpecificData_v1::GetPronounBaseForm(bool vectNumb, Int32 pronNumb) const{
	return GetPronounForm(vectNumb, pronNumb, false, true);
}

// Приватная функция, через которую работают две предыдущие
const MCHAR * LanguageSpecificData_v1::GetPronounForm(bool vectNumb, Int32 pronNumb, bool reverse, bool baseForm )const{
...
Int32 LanguageSpecificData_v1::GetSubtablesNumber()const{
	return sizeof(subtablesNames)/sizeof(char*);
}

// Выдаем одной из таблиц, к которой принадлежат нужные формы глаголов.
const MCHAR * LanguageSpecificData_v1::GetSubtableName(Int32 numb)const{
...
void DelAccentsInSpanishWord_v1(MCHAR * word){
...
bool LanguageSpecificData_v1::IsWordGermDetachablePrefix(const MCHAR* Word) const
{
	if ( !Word || !(*Word) )
		return false;
...
bool MorphoData_v1::LanguageSpecificTrials ( BaseFormsIterator &iter, MCHAR *baseFormBuf,  const MorphoInflectionRulesSetHandle** rulesetPtr, UInt32 language ) const
{
	...
	//	DelAccentsInSpanishWord(wordForm);
		...
		for(Int32 i=2; i--; ){
			...
			for (Int32 j=0; j<langSpecData.GetPronounsVectorSize(i); j++){
				...
				if (!sld2::StrNCmp(reversedWord, (MCHAR*)rp, sld2::StrLen(rp))){
